...
import { AlertTriangle, Camera, Edit, Eye, MapPin, Plus, Save, Search, Sparkles, X } from 'lucide-react';
...
function wordCode(nombre = '') {
  const words = normalize(nombre).replace(/[^A-Z0-9 ]/g, ' ').trim().split(/\s+/).filter(Boolean);
...
export default function ArticulosV3() {
  const {
    articulos,
    proveedores,
    sectores,
  ...
  function generarCodigo(sector, nombre, lista) {
    ...
    const base = `${prefix}-${wordCode(nombre)}`;
  ...
  const articulosFiltrados = articulos.filter((art) => {
    ...
    const texto = normalize(`${art.codigo} ${art.qr || ''} ${art.codigoBarras || ''} ${art.ean || ''} ${art.nombre} ${art.categoria} ${art.marca} ${art.modelo} ${art.ubicacion} ${art.proveedorPrincipal}`);
  ...
  const abrirEditar = (art) => {
    ...
    setForm({
      ...initialForm,
      ...art,
      codigoBarras: art.codigoBarras || art.ean || '',
      stockActual: Number(art.stockActual) || 0,
  ...
  const guardar = async (event) => {
    ...
    const eanFinal = String(form.codigoBarras || '').trim();
    ...
    const codigoDuplicado = articulos.some((art) => art.id !== editando?.id && normalize(art.codigo) === normalize(codigoFinal));
    if (codigoDuplicado) return setError('Ese código interno ya existe.');

    const eanDuplicado = eanFinal && articulos.some((art) => art.id !== editando?.id && normalize(art.codigoBarras || art.ean) === normalize(eanFinal));
    if (eanDuplicado) return setError('Ese código de barras ya está asociado a otro artículo.');

    const payload = {
      ...form,
      codigo: codigoFinal,
      qr: codigoFinal,
      codigoBarras: eanFinal,
      ean: eanFinal,
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {hayFiltros ? `${articulosFiltrados.length} artículos encontrados con filtros activos` : 'Alta rápida con código automático, código de barras, imagen ampliable y entrada inicial.'}
          </p>
        </div>
      ...
      <div className="rounded-3xl border border-gray-100 bg-white p-5 shadow-xs">
        <div className="grid gap-3 md:grid-cols-[1.3fr_0.8fr_0.8fr_0.8fr] xl:grid-cols-[1.5fr_0.8fr_0.8fr_0.8fr_0.8fr_auto]">
          <div className="relative">
            ...
            <input value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 pl-10 pr-4 text-sm outline-none focus:border-amber-500" placeholder="Buscar por código, EAN, nombre, marca, proveedor o ubicación..." />
          </div>
          <Select value={sectorFiltro} onChange={cambiarSectorFiltro} options={sectoresBase} empty="Todos los sectores" />
          <Select value={proveedorFiltro} onChange={setProveedorFiltro} options={proveedoresBase} empty="Todos los proveedores" />
          <select value={stockFiltro} onChange={(e) => setStockFiltro(e.target.value)} className="field bg-gray-50 font-bold">
          ...
          <Select value={ubicacionFiltro} onChange={setUbicacionFiltro} options={ubicacionesBase} empty="Todas las ubicaciones" />
          {hayFiltros && <button onClick={limpiarFiltros} className="rounded-xl bg-gray-100 px-4 py-3 text-xs font-black text-gray-600 hover:bg-gray-200">Limpiar</button>}
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {articulosFiltrados.map((art) => {
          ...
          const foto = art.foto || imageService.placeholder(art.nombre);
          return (
            <article key={art.id} className={`rounded-3xl border bg-white p-5 shadow-xs ${bajo ? 'border-red-100' : 'border-gray-100'}`}>
              <div className="flex items-start gap-4">
                <button type="button" onClick={() => setImagenAmpliada({ src: foto, title: art.nombre })} className="group relative">
                  <img src={foto} alt={art.nombre} className="h-16 w-16 rounded-2xl border border-gray-100 object-cover" />
                  <span className="absolute inset-0 hidden items-center justify-center rounded-2xl bg-black/35 text-white group-hover:flex"><Eye className="h-5 w-5" /></span>
                </button>
                <div className="min-w-0 flex-1">
                  ...
                  {(art.codigoBarras || art.ean) && <p className="mt-1 font-mono text-[11px] font-black text-gray-500">EAN: {art.codigoBarras || art.ean}</p>}
                </div>
          ...
          <div className="max-h-[92vh] w-full max-w-3xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between bg-gray-950 px-6 py-5 text-white">
              <div>
                ...
                <p className="text-xs font-semibold text-gray-400">Código interno automático + código de barras opcional.</p>
              </div>
            ...
            <form onSubmit={guardar} className="max-h-[calc(92vh-88px)] space-y-5 overflow-y-auto p-6">
              ...
              <div className="rounded-3xl border border-amber-100 bg-amber-50 p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-xs font-black uppercase tracking-wider text-amber-700">Código interno sugerido</p>
                    <p className="mt-1 font-mono text-xl font-black text-gray-950">{codigoSugerido}</p>
                  </div>
                  <button type="button" onClick={() => setField('codigo', codigoSugerido)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-gray-950 px-4 py-3 text-xs font-black text-white">
                    <Sparkles className="h-4 w-4 text-amber-400" /> Usar código
                  </button>
                </div>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Código de barras / EAN" help="Opcional. Puedes escribir o pegar el código del fabricante."><input value={form.codigoBarras || ''} onChange={(e) => setField('codigoBarras', e.target.value.trim())} placeholder="8431234567890" className="field font-mono" /></Field>
              ...
              {form.foto && <img src={form.foto} alt="Vista previa" className="h-36 w-48 rounded-2xl border border-gray-100 object-cover" />}
      ...
      {imagenAmpliada && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={() => setImagenAmpliada(null)}><div className="max-w-4xl rounded-3xl bg-white p-3 shadow-2xl" onClick={(e) => e.stopPropagation()}><div className="mb-2 flex items-center justify-between gap-4 px-2"><h3 className="font-black text-gray-900">{imagenAmpliada.title}</h3><button onClick={() => setImagenAmpliada(null)} className="rounded-xl bg-gray-100 p-2"><X className="h-5 w-5" /></button></div><img src={imagenAmpliada.src} alt={imagenAmpliada.title} className="max-h-[75vh] w-full rounded-2xl object-contain" /></div></div>}
    </div>
  );
}

function Select({ value, onChange, options, empty }) { return <select value={value} onChange={(e) => onChange(e.target.value)} className="field bg-gray-50 font-bold"><option value="">{empty}</option>{options.map((item) => <option key={item} value={item}>{item}</option>)}</select>; }
function Chip({ label }) { return <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-amber-700 ring-1 ring-amber-100">{label}</span>; }
function Field({ label, help, children }) { return <label className="block"><span className="label-mini">{label}</span>{children}{help && <span className="mt-1 block text-[11px] font-semibold text-gray-400">{help}</span>}</label>; }
function Info({ label, value, danger = false }) { return <div className="rounded-2xl bg-gray-50 p-3"><p className="text-[10px] font-black uppercase tracking-wider text-gray-400">{label}</p><p className={`mt-1 text-lg font-black ${danger ? 'text-red-600' : 'text-gray-900'}`}>{value}</p></div>; }
