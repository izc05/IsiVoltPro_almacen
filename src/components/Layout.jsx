    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row relative">
      ...
      `}>
            ...
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.badge && (
                    <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${isActive ? 'bg-white text-amber-700' : 'bg-red-500/20 text-red-400'}`}>
                      {item.badge}
                    </span>
                  )}
                </button>
        ...
        <div className="p-4 border-t border-gray-800 bg-gray-950/40 text-center text-xs text-gray-500">
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-center gap-1 rounded-lg px-2 py-1.5 text-[10px] font-bold ${viewMode === item.id ? 'bg-amber-500 text-gray-950' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
                  >
          ...
          <br />
          Modo LocalStorage
          <button onClick={logout} className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-gray-900 px-3 py-2 text-xs font-bold text-gray-300 hover:bg-gray-800">
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-gray-900 border-t border-gray-800 text-white h-16 flex items-center justify-around z-30 shadow-lg px-2 no-print">
        {mobileMenuItems.slice(0, 5).map((item) => {
          ...
          const isActive = activeTab === item.id;
          return (
            <button key={item.id} onClick={() => navigateTo(item.id)} className={`flex flex-col items-center justify-center w-12 h-12 rounded-xl relative ${isActive ? 'text-amber-500' : 'text-gray-400'}`}>
              <Icon className="h-5 w-5" />
              <span className="text-[10px] mt-1 font-medium tracking-tight truncate max-w-full">{item.label.split(' ')[0]}</span>
              {item.badge && <span className="absolute top-1 right-1 bg-red-600 text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold scale-90">{item.badge}</span>}
            </button>
          );
        })}
        <button onClick={() => navigateTo('qr')} className={`flex flex-col items-center justify-center w-14 h-14 -translate-y-4 rounded-full border-4 border-gray-900 shadow-xl relative transition-transform ${activeTab === 'qr' ? 'bg-amber-600 text-white scale-105' : 'bg-amber-500 text-gray-950 hover:bg-amber-600'}`}>
