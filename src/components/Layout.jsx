...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row relative">
      <AnimatePresence>
        {notification && (
          <motion.div initial={{ opacity: 0, y: -50, scale: 0.9 }} animate={{ opacity: 1, y: 16, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.9 }} className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 px-5 py-3 rounded-xl shadow-xl flex items-center space-x-3 text-white max-w-md w-[90%] font-medium ${notification.tipo === 'error' ? 'bg-red-600' : notification.tipo === 'info' ? 'bg-blue-600' : 'bg-amber-600'}`}><span className="flex-1 text-center md:text-left">{notification.mensaje}</span></motion.div>
        )}
      </AnimatePresence>
      ...
      <header className="md:hidden bg-gray-900 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-30 shadow-md no-print">
        <div className="flex items-center space-x-2"><div className="bg-amber-500 p-1.5 rounded-lg"><QrCode className="h-5 w-5 text-gray-950" /></div><span className="font-bold text-lg tracking-wide text-amber-500">IsiVoltPro</span><span className="font-light text-xs text-gray-400">Almacén</span></div>
        <div className="flex items-center space-x-3">{stockBajoCount > 0 && <div className="relative"><Bell className="h-5 w-5 text-amber-500" /><span className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">{stockBajoCount}</span></div>}<button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-1 rounded-lg hover:bg-gray-800 text-gray-300">{sidebarOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}</button></div>
      </header>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-gray-900 text-white flex flex-col justify-between transform transition-transform duration-300 ease-in-out no-print md:translate-x-0 md:static md:h-screen md:flex-shrink-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div>
          <div className="p-6 border-b border-gray-800 flex items-center space-x-3"><div className="bg-amber-500 p-2 rounded-xl shadow-md"><QrCode className="h-6 w-6 text-gray-950" /></div><div><h1 className="font-extrabold text-xl tracking-tight text-white m-0 leading-none">IsiVolt<span className="text-amber-500">Pro</span></h1><p className="text-gray-400 text-xs mt-1 uppercase font-semibold tracking-wider">Almacén e Inventario</p></div></div>
        ...
        <div className="p-4 border-t border-gray-800 bg-gray-950/40 text-center text-xs text-gray-500">
          <div className="mb-3 rounded-xl border border-gray-800 bg-gray-900 p-1"><div className="mb-1 px-2 text-left text-[10px] font-black uppercase tracking-wider text-gray-500">Vista</div><div className="grid grid-cols-3 gap-1">{[{ id: 'auto', label: 'Auto', icon: Monitor }, { id: 'pc', label: 'PC', icon: Monitor }, { id: 'mobile', label: 'Móvil', icon: Smartphone }].map((item) => { const Icon = item.icon; return <button key={item.id} onClick={() => setModoVista(item.id)} className={`flex items-center justify-center gap-1 rounded-lg px-2 py-1.5 text-[10px] font-bold ${viewMode === item.id ? 'bg-amber-500 text-gray-950' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}><Icon className="h-3 w-3" />{item.label}</button>; })}</div></div>
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-x-hidden pb-20 md:pb-0"><div className={`flex-1 p-4 md:p-8 w-full ${mainWidthClass}`}>{children}</div></main>
