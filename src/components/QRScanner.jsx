    ...
    <div className={`space-y-6 ${inlineMode ? '' : 'mx-auto max-w-6xl'}`}>
      {!inlineMode && (
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          ...
          <label className="flex items-center gap-2 rounded-2xl border border-gray-100 bg-white px-4 py-3 text-xs font-black text-gray-600 shadow-xs">
      ...
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="rounded-3xl border border-gray-100 bg-white p-5 shadow-xs">
            ...
            <div className="mt-4 flex gap-2">
              <div className="relative flex-1">
                ...
                <input value={codigoManual} onChange={(e) => setCodigoManual(e.target.value.toUpperCase())} placeholder="Ej. ELE-CABLE-25" className="w-full rounded-xl border border-gray-200 bg-gray-50 py-2 pl-9 pr-4 font-mono text-xs outline-none focus:border-amber-500" />
              </div>
        ...
        <section className="rounded-3xl border border-gray-100 bg-white p-5 shadow-xs lg:min-h-[520px]">
          {!articulo ? (
            <div className="flex h-full min-h-[340px] flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-8 text-center">
              <Camera className="h-12 w-12 text-gray-300" />
              <h3 className="mt-4 text-lg font-black text-gray-900">Listo para registrar</h3>
              <p className="mt-1 max-w-sm text-sm text-gray-500">Busca un artículo por código o selecciona uno de la lista rápida.</p>
              {codigoDetectado && !articulo && (
                <button onClick={() => setActiveTab('articulos')} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2 text-xs font-black text-gray-950">
                  <Plus className="h-4 w-4" /> Crear artículo {codigoDetectado}
                </button>
              )}
            </div>
          ) : (
            <form onSubmit={guardarOperacion} className="space-y-4">
              ...
              {accion === 'recuento' ? <NumberInput label="Stock físico real" value={stockFisico} onChange={setStockFisico} min="0" /> : <NumberInput label="Cantidad" value={cantidad} onChange={setCantidad} min="1" />}
              ...
              {accion === 'entrada' && (
                <label className="block"><span className="label-mini">Proveedor</span><select value={proveedor} onChange={(e) => setProveedor(e.target.value)} className="field"><option value="General">General</option>{proveedores.map((prov) => <option key={prov.id} value={prov.nombre}>{prov.nombre}</option>)}</select></label>
              )}
              ...
              <div className="flex flex-col gap-2 border-t border-gray-100 pt-4 sm:flex-row sm:justify-end">
                <button type="button" onClick={listoSiguiente} className="rounded-xl bg-gray-100 px-5 py-3 text-sm font-black text-gray-600 hover:bg-gray-200"><RefreshCw className="mr-2 inline h-4 w-4" />Siguiente</button>
                <button type="submit" className="rounded-xl bg-amber-500 px-6 py-3 text-sm font-black text-gray-950 hover:bg-amber-400">Guardar</button>
              </div>
            </form>
          )}
        </section>
